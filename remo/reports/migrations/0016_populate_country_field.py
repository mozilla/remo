...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        report.
        """
        Report = orm['reports.NGReport']
        events_reports = Report.objects.filter(event__isnull=False)

        for report in events_reports:
            report.country = report.event.country
            report.save()

        events_reports_ids = events_reports.values_list('id', flat=True)
        reports = Report.objects.exclude(id__in=events_reports_ids)

        for report in reports:
            try:
                country = report.location.split(',')[-1].strip()
            except IndexError:
                pass
    ...
    def backwards(self, orm):
        """ Backwards method - do nothing please."""
        pass
