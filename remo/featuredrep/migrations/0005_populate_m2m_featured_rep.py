...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Populate new m2m field for User."""
        for featured_rep in orm['featuredrep.FeaturedRep'].objects.all():
            featured_rep.users.add(featured_rep.user)
            featured_rep.save()

    def backwards(self, orm):
        """Remove m2m entry for User."""
        for featured_rep in orm['featuredrep.FeaturedRep'].objects.all():
            for user in featured_rep.users.all():
                featured_rep.users.remove(user)
