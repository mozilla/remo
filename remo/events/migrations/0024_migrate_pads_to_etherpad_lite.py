...
PAD_RGX = re.compile('(http|https)://(.*)etherpad.mozilla.org/(.*)')


class Migration(DataMigration):

    def forwards(self, orm):
        """ Migrate the planning pad in events to the new etherpad url."""
        events = orm['events.Event'].objects.all()
        for event in events:
            match = PAD_RGX.match(event.planning_pad_url)
            new_planning_pad = ''
            if match:
                try:
