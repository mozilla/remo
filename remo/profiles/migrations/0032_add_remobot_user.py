...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        user = (orm['auth.User'].objects.create(username='remobot',
                                                email='reps@mozilla.org',
                                                first_name='ReMo',
                                                last_name='bot'))
        user.groups.add(orm['auth.Group'].objects.get_or_create(name='Admin')[0])
        (orm['profiles.UserProfile'].objects.create(user=user,
                                                    registration_complete=True,
                                                    irc_name='remobot',
                                                    display_name='ReMo bot',
                                                    date_joined_program='2007-07-01',
                                                    receive_email_on_add_report=False,
                                                    receive_email_on_edit_report=False,
                                                    receive_email_on_add_comment=False,
                                                    receive_email_on_add_event_comment=False))

    def backwards(self, orm):
        "Write your backwards methods here."
        orm['auth.User'].objects.filter(username='remobot').delete()
    ...
    models = {
        ...
        'profiles.useravatar': {
            ...
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'last_update': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 6, 10, 0, 0)', 'auto_now': 'True', 'blank': 'True'}),
