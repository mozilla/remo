...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """ Make all unavailable users of the Alumni group available again."""
        alumni_statuses = (orm['profiles.UserStatus'].objects
                           .filter(user__groups__name='Alumni')
                           .filter(is_unavailable=True))
        today = datetime.datetime.now()
        for status in alumni_statuses:
            status.is_unavailable = False
            status.return_date = today
            status.save()
    ...
    models = {
        ...
        'profiles.useravatar': {
            ...
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'last_update': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2015, 6, 7, 0, 0)', 'auto_now': 'True', 'blank': 'True'}),
        ...
        'profiles.userstatus': {
            ...
            'start_date': ('django.db.models.fields.DateField', [], {'default': 'datetime.datetime(2015, 6, 8, 0, 0)', 'blank': 'True'}),
