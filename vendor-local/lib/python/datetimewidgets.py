#
# SelectTimeWidget and SplitSelectDateTimeWidget
#
# Original from:
# - http://djangosnippets.org/snippets/1206/
# - http://bradmontgomery.blogspot.gr/2008/11/extending-djangos-multiwidget.html
#
...
class SelectTimeWidget(Widget):
    ...
    def __init__(self, attrs=None, hour_step=None, minute_step=None,
                 second_step=None, twelve_hr=False, use_seconds=False):
        """
        hour_step, minute_step, second_step are optional step values for
        for the range of values for the associated select element
        twelve_hr: If True, forces the output to be in 12-hr
        """
        self.attrs = attrs or {}
    ...
    def render(self, name, value, attrs=None):
        ...
        select_html = (Select(choices=minute_choices)
                       .render(self.minute_field
                               % name, minute_val, local_attrs))
        output.append(select_html)
        ...
        if self.use_seconds:
            ...
            select_html = (Select(choices=second_choices)
                           .render(self.second_field
                                   % name, second_val, local_attrs))
            output.append(select_html)
        ...
        if self.twelve_hr:
            ...
            select_html = (Select(choices=meridiem_choices)
                           .render(self.meridiem_field
                                   % name, self.meridiem_val, local_attrs))
            output.append(select_html)
...
class SplitSelectDateTimeWidget(MultiWidget):
    ...
    django.forms.widgets), but with Select elements.
    """
    def __init__(self, attrs=None, hour_step=None, minute_step=None,
